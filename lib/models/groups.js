...
authentication, only public groups are returned..
     ...
     * @param params.all_available {boolean} - Show all the groups you have access to (defaults to false for authenticated users)
     * @param params.search {string} - Return the list of authorized groups matching the search criteria
     * @param params.order_by {string} - Order groups by name or path. Default is name
     ...
     * @param params.owned {boolean} - Limit to groups owned by the current user
     * @returns {*}
     */
...
public projects are returned..
    ...
    listAGroupProjects(id, params) { // eslint-disable-line camelcase
...
lfs_enabled (optional) {boolean} - Enable/disable Large File Storage (LFS) for the projects in this group
    ...
    searchForGroup(params) {
        return this.http.get(`/groups?search=foobar`, params); // eslint-disable-line quotes
    }
    ...
    namespacesInGroups(params) {
        return this.http.get(`/groups?per_page=100`, params); // eslint-disable-line quotes
    }
